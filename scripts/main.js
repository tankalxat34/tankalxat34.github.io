...
function getData(url, resolve, ...resolveArgs) {
    fetch(url)
        .then(response => response.json())
        .then(json => resolve(json, ...resolveArgs))
...
function resolve(json, ...resolveArgs) {
    ...
    for (let element of json) {
        ...
        if (!element.name.toLowerCase().includes("learning")) {
            if (element.html_url != "") {
                local_html_url = `window.open('${element.html_url}')`
            } else {
                local_html_url = "alert('К этому проекту не предусмотрен подробный просмотр')"
            }
            html += 
`
<div class="${divClassName}" id="${element.node_id}" onclick="${local_html_url}" title="Открыть проект ${element.name}">
    <h3>${element.name}</h3>
...
`
    ...
    document.getElementById(projectsPlaceName).innerHTML = html
}
...
class Requests {
    constructor(url, typeOfRequest="GET", returnOnlyAfterResponse=true) {
        this.url = url
        this.typeOfRequest = typeOfRequest
        this.returnOnlyAfterResponse = !returnOnlyAfterResponse

        this.req = new XMLHttpRequest();
        this.req.open(this.typeOfRequest, this.url, this.returnOnlyAfterResponse);
    }
...
function downloadProjects() {
    // getData("https://api.github.com/users/tankalxat34/repos", resolve, divClassName="projects block", projectsPlaceName="projects-place")
    let r = new Requests("https://api.github.com/users/tankalxat34/repos")
    resolve(r.json(), divClassName="projects block", projectsPlaceName="projects-place")
}

function downloadSpecialProjects() {
    // getData("https://raw.githubusercontent.com/tankalxat34/tankalxat34.github.io/content/specialProjects.json", resolve, divClassName="projects block", projectsPlaceName="special-projects-place")
    let r = new Requests("https://raw.githubusercontent.com/tankalxat34/tankalxat34.github.io/content/specialProjects.json")
    resolve(r.json(), divClassName="projects block", projectsPlaceName="special-projects-place")
}

function activeHTMLPath() {
    return window.location.href.split("/").reverse()[0]
}


fillHeader()
if (activeHTMLPath() === "projects.html") {
    downloadProjects()
    downloadSpecialProjects()
}
fillFooter()
